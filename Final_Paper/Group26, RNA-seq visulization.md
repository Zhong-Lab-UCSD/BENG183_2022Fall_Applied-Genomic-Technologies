...
**Figure 5.** This image depicts a PCA plot of the first two PCs on 175 RNA-seq samples obtained from 20 patients with prostrate cancer who underwent different types of androgen deprivation therapy (ADT) treatments and RNA-seq samples include pre-ADT biopsies and post-ADT prostatectomy specimens.
 ...
 - **Input:**
 To generate a heat map, we need to input the differential expression file after the differential analysis step. Each data point in a heat map is a column of 3 elements, top differetially expressed genes, sample names, and normalized read counts of transcripts. It is ussually displayed as a table.
...
**Table 1** It is an example of a differential expression file. The numbers represents the normalizd read counts.

 ### Data Intepretation 

Each rectangle in a heat map represents a data point. The color measures the expression level of the genes. In the example below, red color represents that the gene is upregulated in the sample compared to the other sample, and the blue measn the opposite. 
 ...
 **Figure 6.** An example of a heat map generated by RNA-seq data. This heat map is after clustering. 
 ...
 The intensity of the coloe reflects the magnitide of the differential expression. For example, a deeper red color means that this gene is even more upregulated in the sample compared to the samples with lighter red. 
